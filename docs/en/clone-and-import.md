...
`git clone https://github.com/ByteLegendQuest/java-how-many-days-in-month`

A project directory named `java-how-many-days-in-month` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-how-many-days-in-month` directory which is created just now.
